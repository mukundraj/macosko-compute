...
common_setup(){
       ...
       micromamba run -n "$envname" jupyter lab --generate-config && echo -e 'c.ServerApp.terminals_enabled = True\nc.FileContentsManager.delete_to_trash = False\nc.ContentsManager.allow_hidden = True' >> /jupyter/.jupyter/jupyter_lab_config.py
        ...
        micromamba run -n "$envname" R -e 'if (file.exists("renv/activate.R")) { renv::load(); IRkernel::installspec(user = FALSE) } else if (file.exists("renv.lock")) { renv::init(bare=TRUE); renv::restore(); renv::install(c("pbdZMQ", "IRkernel", "yaml")); IRkernel::installspec(user = FALSE) } else { renv::init(); renv::install(c("pbdZMQ","IRkernel", "yaml")); IRkernel::installspec(user = FALSE) }'
